...
def _make_image_client() -> InferenceClient:
...
def gen_image(prompt: str):
    ...
    try:
        ...
        if any(x in msg for x in ["401", "Unauthorized", "Invalid username or password"]):
            token_present = bool(config.get_hf_api_key())
            return None, (
                "❌ Hugging Face auth failed (401 Unauthorized).\n\n"
                + (
                    "A token was detected but rejected.\n"
                    "This usually means the token is invalid/revoked, missing required inference permissions, or model access terms are not accepted.\n\n"
                    if token_present
                    else "No token was detected in app runtime.\n\n"
                )
                + "Fix:\n"
                "1. Create/regenerate a HF token and use it as `HF_API_KEY` (or `HF_TOKEN`).\n"
                "2. Ensure the token includes inference access and restart Streamlit.\n"
                "3. Accept model access terms if required: https://huggingface.co/stabilityai/stable-diffusion-3-medium-diffusers\n"
                "4. Token settings: https://huggingface.co/settings/tokens\n"
...
def main():
    ...
    if not config.get_hf_api_key():
        st.warning(
            "HF token not found. Set `HF_API_KEY` (or `HF_TOKEN`) to avoid 401 errors during image generation."
        )
